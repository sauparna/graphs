...
int TestHeadAndTailLinks() {
  if(head->llink->rlink == head) {
    fprintf(stderr, "  PASS: The head's llink points to the tail and the tail's rlink points to the head.\n");
    return PASS;
  } else {
...
void TestPriorityQueue() {
  ...
  fprintf(stderr, "Inserted 1023 vertices into the priority queue, where each element's dist is in ascending order, in [1, 1023].\n");

  fprintf(stderr, "%d. Test the queue structure after 1023 enqueue operations.\n", ++test_count);
  ...
  if (TestHeadAndTailLinks() == FAIL) return;  
  fprintf(stderr, "Insert an element with dist = 0.\n");
  (*enqueue)(&v[0], 0);
  if (head->rlink->dist == 0) {
    fprintf(stderr, "  PASS: enqueue queued the smallest element correctly; the first element has dist = 0.\n");
  } else {
    fprintf(stderr, "  FAIL: The smallest element enqueue queued could not be found at the correct location!\n");
    return;
  }
  if (TestHeadAndTailLinks() == FAIL) return;

  fprintf(stderr, "The queue looks structurally sound, which asserts that enqueu works correctly.\n");
  ...
  fprintf(stderr, "%d. Test the requeue operation.\n", ++test_count);
  {
    ...
    if (requeued_element == old_tail) {
      fprintf(stderr, "  PASS: The tail element was requeued to the correct position.\n");
    } else {
      fprintf(stderr, "  FAIL: The tail element is not to be found in the expected position after a requeue!\n");
      return;
    }
    if (requeued_element->dist == 2) {
      fprintf(stderr, "  PASS: The requeued element's dist is correct.\n");
    } else {
      fprintf(stderr, "  FAIL: The requeued element's dist was not correctly set.\n");
      return;
    }
    if (TestHeadAndTailLinks() == FAIL) return;
  }
  ...
  fprintf(stderr, "%d. Test the del_min operation.\n", ++test_count);
  {
    ...
    if (head->rlink == old_second_element) {
      ...
      fprintf(stderr, "  FAIL: del_min left the queue in an inconcistent state!\n");
      return;
    }
    if (TestHeadAndTailLinks() == FAIL) return;
  }
  ...
  fprintf(stderr, "OK, the priority queue implementation in GB_DIJK seem to work! (Passes all %d tests.)\n", test_count);
}
